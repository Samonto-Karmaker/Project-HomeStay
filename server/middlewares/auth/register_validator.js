...
const registerValidator = [
    ...
    check("mobile")
        ...
        .custom(async (value) => {
            try {
                ...
                if (user) {
                    throw createError(
                        "MobileError: Mobile number already in use"
                    );
                }
            } catch (error) {
                throw createError(error.message);
    ...
    check("password")
        .isStrongPassword()
        .withMessage(
            "PasswordError: Password must be at least 8 characters long and should contain at least 1 lowercase, 1 uppercase, 1 number and 1 symbol"
        ),
];
